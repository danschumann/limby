var
  ...
  getArgNames = require('./get_argument_names'),
...
module.exports = {
    ...
    _.each(limby.limbs, function(branch, branchName) {
      ...
      _.each(keys || ['models', 'controllers', 'mailers'], function(folder){
        ...
        var unwrapSingle = function(val, key, container) {
          if (!val) {
            console.log('File has no module.exports'.red, val, key);
            throw new Error('File has no module.exports')
          };
          ...
          debug('_require', val, key);
          var output = limby._require(path, {models: branch.models});
          debug('_required', val, key);

          if (folder == 'models' || folder == 'middleware') {
            delete container[key];
            // fileOutput is {  MyModel: limby.Model..., MyCollection: bookshelf.Collection... }
            _.extend(container, output);
          } else
  ...
  require: function(lPath) {
    ...
    return limby._require(output, {parent: parent, localKey: localKey, models: limb.models});
  },

  // TODO: Allow this to require files if they've not been loaded yet (fix race condition)
  // This method requires their file and checks
  // to see if they wrapped it in `function(limby, [models]) { ... }`
  // if they did, it unwraps it
  _require: function(output, options) {

    var parent, localKey, models;
    ...
    if (_.isString(output)) {
      ...
      // If this was nested on an object, we replace it
      if (parent && localKey) parent[localKey] = output;
      ...
      if (_.isFunction(output)) {
        ...
        if (argNames[0] == 'limby') {
          output = output(limby, argNames[1] == 'models' ? models : null );
          if (parent && localKey) parent[localKey] = output;
        };
