...
function setupPaths(limby) {
  ...
  limby.paths.views   = limby.paths.views   || join(limby.paths.core, 'views');
  limby.paths.uploads = limby.paths.uploads || join(limby.paths.core, 'public/uploads');

  _.each(['core', 'limbs', 'views', 'uploads'], function(key) {
  ...
  // and store the absolute paths
  limby.paths.limbs = join(limby.root, limby.paths._limbs);
  limby.paths.views = join(limby.root, limby.paths._views);
