module.exports = function (original, canvas) {
  ...
  // this prevents weird scan lines when doubles get rounded off
  ...
  for (var i = 0; i < (_data2.length >> 2); i++){
    ...
    //data2[i<<2] /= alphas[i];     // r
    //data2[(i<<2) + 1] = (data2[(i<<2) + 1] + data2[(i<<2) + 2]) / 2; // g
    //data2[(i<<2) + 2] = (data2[(i<<2) + 1] + data2[(i<<2) + 2]) / 2; // g
  }
