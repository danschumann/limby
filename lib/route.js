...
module.exports = {
  ...
  route: function() {
    ...
    return when.all(_.map(limby.limbs, function(branch, branchName) {
      ...
      if (http && (http.port || http.host)) {
        ...
        limby.httpServer = limby.server = require('http').createServer(app);
        limby.httpServer.listen.apply(limby.httpServer, args);
      }
      ...
      if (https && (https.port || https.host)) {
        ...
        limby.httpsServer = require('https').createServer(https.certificate, app);
        limby.httpsServer.listen.apply(limby.httpsServer, args);
      }
  ...
  extend: function(limbName) {
    ...
    if (limb.stylesheets)
      promises.push(
        limby.middleware.stylus(_.extend(limbConfig.stylus || {}, {
          app: app,
          baseURL: '/' + limbUrl,
          limbName: limbName,
